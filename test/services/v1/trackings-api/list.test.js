...
describe('trackings-list-v1', function () {
	...
	it('returns results in a paginated way', async () => {
		...
		let response = await handler({ body: JSON.stringify({ userId: "user1", page: 1, per_page: 2 }) })
		expect(response.statusCode).to.equal(200)
		let body = JSON.parse(response.body)
		expect(body.totalHits).to.equal(6)
		expect(body.total).to.equal(2)
		expect(body.page).to.equal(1)
		expect(body.per_page).to.equal(2)
		...
		response = await handler({ body: JSON.stringify({ userId: "user1", page: 2, per_page: 3 }) })
		expect(response.statusCode).to.equal(200)
		body = JSON.parse(response.body)
		expect(body.totalHits).to.equal(6)
		expect(body.total).to.equal(3)
		expect(body.page).to.equal(2)
		expect(body.per_page).to.equal(3)
		expect(body.trackings.length).to.equal(3)
